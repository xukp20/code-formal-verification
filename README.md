...
The input to the pipeline is the source code of the project and the API doc of the project. All the example projects can be downloaded from [here](https://anonymous.4open.science/r/code-formal-verification-data-D531), in the `source_code.zip` file.
...
**Note:** When using the provided outputs from the data repository for evaluation, ensure you have Lean version `4.17.0` installed, as this was the version used to generate the outputs. This version requirement can be ignored if you are running the pipeline from scratch.
...
#### Statistics of the theorem proving results

The correctness of the code formalization and theorem generation are evaluated by hand, and here we only provide the code to automatically generate the statistics of the theorem proving results, which is in `src/tools/theorem_analyzer.py`.

You can use the `analyze.sh` script to generate the statistics of the theorem proving results, and the results will be saved in the `theorem_stats` folder.

#### Visualization of the theorem proving results

The visualization of the theorem proving results is in the `visualization_output` folder, which can be generated by the `plot.sh` script, using the `src/tools/visualization.py` tool.
